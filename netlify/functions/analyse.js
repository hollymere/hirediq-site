...
exports.handler = async function(event, context) {
  ...
  // STEP 2: KEYWORDS + QUESTIONS
  if (mode === 'keywords') {
    const prompt = `List ATS keywords and give 3 specific interview questions. ASCII only, no smart quotes or em dashes. Keep all text concise.
Respond ONLY with this JSON:
{
  "ats_keywords_present": ["keyword1", "keyword2"],
  "ats_keywords_missing": ["keyword1", "keyword2"],
  "screening_questions": [
    {"question": "question text", "why_asked": "one sentence", "answer_framework": "one sentence"}
  ]
}
Exactly 3 questions specific to this role and CV gaps. Not generic.
JOB: ${jd}
CV: ${cv}
JSON only.`;
    return makeRequest(prompt, 800);
  }

  // STEP 3: CV REWRITE
  if (mode === 'rewrite') {
    const prompt = `Rewrite this CV tailored for this specific role. ASCII only, no smart quotes or em dashes. Be concise but complete.
Respond ONLY with this JSON:
{
  "ats_optimised_cv": "full rewritten CV with missing keywords added naturally. Use these headers only: Professional Summary, Work Experience, Education, Skills. Tailor every section to this role."
}
JOB: ${jd}
CV: ${cv}
JSON only.`;

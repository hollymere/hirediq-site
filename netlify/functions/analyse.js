...
exports.handler = async function(event, context) {
  ...
  if (mode === 'cover_letter') {
    const prompt = `Write a cover letter. ASCII only, no smart quotes or em dashes.
Respond ONLY with this JSON:
{"cover_letter": "<3 paragraph cover letter for this role>"}
JOB: ${jd}
CV: ${cv}
JSON only.`;
  ...
  // REWRITE - keywords and questions only, no CV text
  if (mode === 'rewrite') {
    const prompt = `List ATS keywords and give 3 interview questions. ASCII only, no smart quotes or em dashes. Keep all answers short.
Respond ONLY with this JSON:
{
...
Exactly 3 questions. All text must be short.
JOB: ${jd}
CV: ${cv}
JSON only.`;
    return makeRequest(prompt, 800);
